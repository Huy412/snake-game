...
message_end_time = pygame.time.get_ticks() + 3000 # đang thử nghiệm
...
def highscore_func(highscore):
    game = custom_font.render("High score : " + str(highscore), True, yellow)
    dis.blit(game,[250,0])
...
def gameLooping_func():
    ...
    while not game_end:
        ...
        while game_close == True:
            ...
            # Thực hiện điểm số của trò chơi tương đương với chiều dài của con rắn - 1 (không tính phần đầu của con rắn)
            score_func(Length_of_snake - 1)
            highscore_func(Length_of_snake - 1)
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_r:
                        gameLooping_func()
                        if score_func(Length_of_snake - 1) > highscore_func(Length_of_snake - 1):
                            highscore_func() == score_func()
                        score_func() == 0
        ...
        if x1 == foodx and y1 == foody:
            font = pygame.font.SysFont('Time News Roman', 30)
            yummy_button = font.render('Yummy!!', True, (128, 0, 128))
            dis.blit(yummy_button, [600, 100])
            foodx = round(random.randrange(0, dis_width - snake_block) / 10.0) * 10.0
            foody = round(random.randrange(0, dis_height - snake_block) / 10.0) * 10.0
            Length_of_snake += 1
            pygame.display.update()
